...
Amdi.applyCMD({pattern: 'sticker$', fromMe: LOL, desc: Lang.STICKER_DESC,  deleteCommand: false}, (async (message, match) => {    
...
Amdi.applyCMD({pattern: 'sticvid$', fromMe: LOL, desc: Lang.ANI_STICK,  deleteCommand: false}, (async (message, match) => {
    ...
    await webp2mp4File(savedFilename).then(async (rest) => {
        await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
            ...
            saving.on("finish", async () => {
                await message.client.sendMessage(msgid, fs.readFileSync('/root/mywhatsappbot/vstic.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: Config.CAP, quoted: message.data , thumbnail: thumb })
